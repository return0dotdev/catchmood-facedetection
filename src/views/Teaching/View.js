    ...
    _saveTeaching() {
        this.setState({
            ...
            const teaching = await teaching_model.endEmotion({
                ...
                link_video: "Video-" + this.state.key_id.teaching_key + ".mp4"
            })
            if (teaching === 'Success') {
                this.props.history.push(`/course/term/${this.state.key_id.course_key}/${this.state.key_id.term_key}`);
            } else {
                    ...
                    <div className="left-column-teaching">
                        <div className="card-topic-teaching" style={{ width: '100%' }}>
                            ...
                            <div className={this.state.show_video ? "load-iframe" : "wait-load-iframe"}>
                                {sessionStorage.getItem("Camera") !== null && this.state.key_id.teaching_key !== '' && this.state.show_video === true ?
                                    // <img style={{ width: '95%', height: '450px' }} src={`http://13.229.233.187:5000/api/camera/open?url=${sessionStorage.getItem("Camera")}&id_teaching=${this.state.key_id.teaching_key}&check_record=${sessionStorage.getItem("Record_Video")}`} />
                                    <img style={{ width: '95%', height: '450px' }} src={`http://127.0.0.1:5000/api/camera/open?url=${sessionStorage.getItem("Camera")}&id_teaching=${this.state.key_id.teaching_key}&check_record=${sessionStorage.getItem("Record_Video")}`} />
                                    : null
                                }
                            </div>
                            <button
                                className="btn btn-sm"
                                disabled={this.state.show_video}
